...
router.get('/formulario/embarcacaoVizu/:id', eUser, async(req, res) => {
    try{
        ...
        req.flash('error_msg', `Erro ao visualizar esta Embarcação (${err})`)
        res.redirect('/formulario')
    }
})
...
router.get('/embarcacoes', eUser, async (req, res) => {
    try{
        ...
        req.flash('error_msg', `Erro ao listar Embarcações (${err})`)
        res.redirect('/formulario')
    }
})
...
router.get('/embarcacoes/:page', eUser, async (req, res) => {
    ...
    const skip = (page - 1) * limit;
     try{
        ...
        req.flash('error_msg', `Erro ao paginar Embarcações (${err})`)
        res.redirect('/formulario')
     }
})
...
router.get('/admin/embarcacoesEdit/:id', Admin, async(req, res) => {
    try{
        ...
        req.flash('error_msg', `Erro ao mostar página de edição de Embarcação (${err})`)
...
router.post('/admin/embarcacoesEdit', Admin, async(req, res) => {
    try{
        ...
        console.log(err)
        req.flash('error_msg', `Erro ao editar Embarcação (${err})`)
        res.redirect('/admin/painel')
    }
})
...
router.get('/admin/embarcacoes', Admin, async (req, res) => {
    try{
        const embarcacoes = await Embarcacao.find().limit(5).lean().sort({ EmbarcacaoNome: 'asc' })
            res.render('admin/embarcacoes/listaEmbarcacoes', 
                { 
                    embarcacoes: embarcacoes 
                })
    } catch(err){
...
router.get('/embarcacao/:id/pdf', Admin, async (req, res) => {
    try{
        const embarcacoes = await Embarcacao.findById(req.params.id).lean()
        const html = `
        <form>
    ...
    ;
        ...
        req.flash('error_msg', `Erro ao gerar PDF desta Embarcação (${err})`)
        res.redirect('/admin/painel')
    }
})
